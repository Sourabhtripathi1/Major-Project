...
exports.getAllListingFilter = async (req, res) => {
  try {
    const { filter } = req.params;
    const regex = new RegExp(filter, "i"); // 'i' for case-insensitive search

    // Construct the query using $or and $regex
    const query = {
      $or: [
        { "location.country.name": { $regex: regex } },
        { "location.state.name": { $regex: regex } },
        { "location.city.name": { $regex: regex } },
      ],
    };
